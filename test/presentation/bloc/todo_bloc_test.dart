...
void main() {
  late MockGetTodo mockGetTodoUsecase;
  late TodoBloc todoBloc;

  setUp(() {
    di.init();
    mockGetTodoUsecase = MockGetTodo();
    todoBloc = TodoBloc();
  });
  ...
  test('initial state should be loading', () {
    expect(TodoBloc().state, TodoState(status: TodoStatus.loading));
  });

  blocTest<TodoBloc, TodoState>(
      'should emit [Success] when data is gotten successfully',
      build: () {
        when(mockGetTodoUsecase.execute(offset))
            .thenAnswer((_) async => Right(testTaskEntities));

        return todoBloc;
      },
      act: (bloc) => bloc.add(TodoQuery()),
      wait: const Duration(milliseconds: 500),
      expect: () => [
            TodoState(
                status: TodoStatus.success,
                todos: testTaskEntities,
                hasReachedMax: false)
          ]);
  ...
  blocTest<TodoBloc, TodoState>(
      ...
      setUp: () {
        when(mockGetTodoUsecase.execute(offset)).thenAnswer(
      ...
      wait: const Duration(milliseconds: 500),
      expect: () => [
            const TodoState(status: TodoStatus.error),
          ]);
}
