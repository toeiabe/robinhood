...
void main() {
  late MockGetDoing mockGetDoingUsecase;
  late DoingBloc doingBloc;

  setUp(() {
    di.init();
    mockGetDoingUsecase = MockGetDoing();
    doingBloc = DoingBloc();
  });
  ...
  test('initial state should be loading', () {
    expect(DoingBloc().state, DoingState(status: DoingStatus.loading));
  });

  blocTest<DoingBloc, DoingState>(
      'should emit [Success] when data is gotten successfully',
      build: () {
        when(mockGetDoingUsecase.execute(offset))
            .thenAnswer((_) async => Right(testTaskEntities));

        return doingBloc;
      },
      act: (bloc) => bloc.add(DoingQuery()),
      wait: const Duration(milliseconds: 500),
      expect: () => [
            DoingState(
                status: DoingStatus.success,
                doings: testTaskEntities,
                hasReachedMax: false)
          ]);
  ...
  blocTest<DoingBloc, DoingState>(
      ...
      setUp: () {
        when(mockGetDoingUsecase.execute(offset)).thenAnswer(
      ...
      wait: const Duration(milliseconds: 500),
      expect: () => [
            const DoingState(status: DoingStatus.error),
          ]);
}
